...
cifar10_classes = {'airplane':0, 'automobile':1, 'bird':2, 'cat':3, 'deer':4, 'dog':5,
                    'frog':6, 'horse':7, 'ship':8, 'truck':9}
# labels = imagenet_classes['truck'] * jnp.ones((4,), dtype=jnp.int32)
for key in cifar10_classes.keys():
    labels = cifar10_classes[key] * jnp.ones((1,),dtype=jnp.int32)
    samples = model.samples_fn(rng=jax.random.PRNGKey(0), labels=labels, params=loaded_params, num_steps=1000)
    ...
    img = plt.imsave('./cifar10_generation/'+key+'.png',img_grid)
