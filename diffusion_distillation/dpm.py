...
class Model:
  ...
  def _run_model(self, *, z, logsnr, model_fn, clip_x):
    ...
    # 四种模型形式 返回的是预测的x 噪声和v的一个词典 可通过下标来选
  ...
  def training_losses(self, *, x, rng, logsnr_schedule_fn,
    ...
    # sample z ~ q(z_logsnr | x) z是一个加噪后某个时间的的图像
